...
CREATE TABLE IF NOT EXISTS client_communications (
    ...
    client_email VARCHAR(255) NOT NULL CHECK (client_email REGEXP '(?i)^[A-Za-z0-9._%+-]+@[A-Za-z0-9.-]+\\\\.[A-Za-z]{2,}$'),
...
CREATE TABLE IF NOT EXISTS user_communications (
    ...
    user_email VARCHAR(255) NOT NULL CHECK (user_email REGEXP '(?i)^[A-Za-z0-9._%+-]+@[A-Za-z0-9.-]+\\.[A-Za-z]{2,}$'),
...
CREATE TABLE IF NOT EXISTS otp_communications (
    id UUID DEFAULT random_uuid() PRIMARY KEY,
    email VARCHAR(255) NOT NULL CHECK (email REGEXP '(?i)^[A-Za-z0-9._%+-]+@[A-Za-z0-9.-]+\\.[A-Za-z]{2,}$'),
    otp INT NOT NULL,
    subject VARCHAR(255) NOT NULL,
    status VARCHAR(50) NOT NULL,
    timestamp TIMESTAMP NOT NULL
